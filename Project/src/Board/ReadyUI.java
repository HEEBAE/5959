...
public class ReadyUI extends JFrame implements ActionListener {
	...
	public ReadyUI(MemberVO vo) {
		...
		try {
			...
			if(Integer.parseInt(vo.getPlaynum())<20) //20ÆÇ¹Ì¸¸
			{}
			else if(Float.parseFloat(vo.getWinningRate())>=80)//´ÙÀÌ¾Æ
			{}
			else if(Float.parseFloat(vo.getWinningRate())>=60)//ÇÃ·¡
			{}
			else if(Float.parseFloat(vo.getWinningRate())>=40)//°ñµå
			{}
			else if(Float.parseFloat(vo.getWinningRate())>=20)//½Ç¹ö
			{}
			else //ºê·ÐÁî
			{}
	...
	public void actionPerformed(ActionEvent e) {
		...
		case "°ÔÀÓ½ÃÀÛ":////////////////////////////////////////////////////////
			...
			this.dispose();
				...
				JOptionPane.showMessageDialog(null, "ÀÌ¹ÌÁ¢¼ÓµÇ¾î ÀÖ´Â °èÁ¤ÀÔ´Ï´Ù.", null, JOptionPane.INFORMATION_MESSAGE);
			}*/
			board.getThread().setUsername(vo.getId());
			new Board(vo);
			this.dispose();
